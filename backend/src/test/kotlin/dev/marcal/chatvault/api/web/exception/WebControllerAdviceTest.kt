...
class WebControllerAdviceTest {
    ...
    class TestExceptionController {
        ...
        fun throwAttachmentNotFoundException(): Unit =
            throw AttachmentNotFoundException("Attachment with id 1 was not found", Exception("cause"))

        @GetMapping("/test-chat-not-found")
        fun throwChatNotFoundException(): Unit = throw ChatNotFoundException("Chat with id 999 was not found")

        @GetMapping("/test-bucket-file-not-found")
        fun throwBucketFileNotFoundException(): Unit =
            throw BucketFileNotFoundException("File not found in bucket", Exception("root cause"))

        @GetMapping("/test-message-parser-exception")
        fun throwMessageParserException(): Unit = throw MessageParserException("Invalid message format")

        @GetMapping("/test-illegal-argument")
        fun throwIllegalArgument(): Unit = throw IllegalArgumentException("Invalid argument provided")

        @GetMapping("/test-illegal-state")
        fun throwIllegalState(): Unit = throw IllegalStateException("Invalid state detected")

        @GetMapping("/test-chat-importer-exception")
        fun throwChatImporterException(): Unit = throw ChatImporterException("Import failed")

        @GetMapping("/test-bucket-service-exception")
        fun throwBucketServiceException(): Unit = throw BucketServiceException("Bucket operation failed", Exception("root cause"))

        @GetMapping("/test-attachment-finder-exception")
        fun throwAttachmentFinderException(): Unit = throw AttachmentFinderException("Attachment not found", Exception("root cause"))
    }
    ...
    fun `should handle BucketFileNotFoundException with 404 status`() {
        // Act & Assert
        mvc
            .get("/test-bucket-file-not-found")
            .andExpect {
                status { isNotFound() }
            }
    }
    ...
    fun `should handle AttachmentFinderException with 404 status`() {
        // Act & Assert
        mvc
            .get("/test-attachment-finder-exception")
            .andExpect {
                status { isNotFound() }
            }
    }
    ...
    fun `should handle MessageParserException with 422 status`() {
        // Act & Assert
        mvc
            .get("/test-message-parser-exception")
            .andExpect {
    ...
    fun `should handle ChatImporterException with 500 status`() {
        // Act & Assert
        mvc
            .get("/test-chat-importer-exception")
            .andExpect {
    ...
    fun `should handle BucketServiceException with 500 status`() {
        // Act & Assert
        mvc
            .get("/test-bucket-service-exception")
            .andExpect {
                status { isInternalServerError() }
            }
    }
